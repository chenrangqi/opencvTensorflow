...
#------划分测试数据和训练数据--------#
train_imgs, test_imgs, train_labels, test_labels = train_test_split( img, lables, test_size = 0.05, random_state = random.randint(0, 100) )

#-------------数据换成图片总数、图片大小、图片通道------------------#
train_imgs = train_imgs.reshape(train_imgs.shape[0], size, size, 3)
test_imgs  = test_imgs.reshape( test.imgs.shape[0] , size, size ,3)
...
keep_prob_75 = tf.placehleder(tf.float32)
...
def cnnTrain():
    ...
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = out, lables = y_))
    #梯度下降法得到步长
    ...
    #tensorBoard可视化
    tf.summary.scaler('loss', cross_enropy)
    ...
    with tf.Session() as sess:
        ...
        for n in range(1):
            for i in range(batch_num):
                ...
                batch_labels = train_labels[i*batch_size : (i+1)*bach_size]
                #开始训练数据，同时训练三个变量，返回三个数据
                _, loss, summary = sess.run([train_step, cross_entropy, merged_summary_op], \
                                    feed_dict = {x:batch_imgs, y:batch_labels, keep_prob_5:0.5, keep_prob_75:0.75})
                print(n*batch_num + i, loss)

            accur = accuracy.eval({x:train_imgs, y_:train_labels, keep_prob_5:1.0, keep_prob_75:1.0})
